...
library(reticulate)
use_python("/path/miniconda3/bin/python", required=TRUE)
#mofa <- import("mofapy2")
...
outfile = file.path(paste0("/path/dp_omics/MOFA_fit_2/1241_4000_3000_200_cph11_nc_", seed,".hdf5"))
#outfile = file.path(paste0("./MOFA_fit_2/1241_4000_3000_200_cph11_nc_", seed,".hdf5"))
MOFAobject.trained <- MOFA2::run_mofa(MOFAobject, outfile,use_basilisk = F)
...
##########Second step
dat <- data_gen_ls(seed*2002, tp, N,Z_hat) ##generate longitudinal and survival data
...
### stan sampling
md = stan_model('source_code/M2_21.stan')
...
inits2 <- list(A1 = g.inits(mu_est[[1]]$coefficient), A2 = g.inits(mu_est[[2]]$coefficient),
               ...
               Y2_imp = as.array(mean_Y[[2]]-rnd), 
               Y3_imp = as.array(mean_Y[[3]]-rnd))
inits3 <- list(A1 = g.inits(mu_est[[1]]$coefficient), A2 = g.inits(mu_est[[2]]$coefficient),
               A3 = g.inits(mu_est[[3]]$coefficient), 
               sqrt_d0 = as.array(g.inits.sd(sqrt(d0_est))), 
               sqrt_d1 = as.array(g.inits.sd(sqrt(d1_est))),
               beta = g.inits(beta_hat[2:J]), 
               ...
               Y2_imp = as.array(mean_Y[[2]]+rnd), 
               Y3_imp = as.array(mean_Y[[3]]+rnd))
inits4 <- list(A1 = g.inits(mu_est[[1]]$coefficient), A2 = g.inits(mu_est[[2]]$coefficient),
               A3 = g.inits(mu_est[[3]]$coefficient), 
               sqrt_d0 = as.array(g.inits.sd(sqrt(d0_est))), 
               sqrt_d1 = as.array(g.inits.sd(sqrt(d1_est))),
               beta = g.inits(beta_hat[2:J]), 
               ...
               Y2_imp = as.array(mean_Y[[2]]-rnd), 
               Y3_imp = as.array(mean_Y[[3]]-rnd))
inits <- list(c1 = inits1, c2 = inits2, c3 = inits3, c4 = inits4)
...
fitStan <- sampling(md, data = stan_dat, iter = 4000, warmup = 3000, 
                    chains = 4, thin=1, init = inits, pars = pars, seed = 2002*seed,
                    control = list(adapt_delta = 0.8, max_treedepth=15))
fname <- paste0("/path/dp_omics/stan_fit_2/1241_4000_3000_200_cph11_nc", args, '.RData',sep="")
...
summary.parameters[(i2+n_total_factors+1):(i2+n_total_factors+1), ] <- get_quantile_vector(gamma0_sample)

i3 <- i2+n_total_factors+1
summary.parameters[(i3+1):(i3+L1*J), ] <- get_quantile_matrix(gamma1_sample)
...
H_true <- function(tp, x, weights, nodes, 
  ...
  H <- exp(tp$logh0 + sum(x*tp$gamma_x))*t/2*sum.approx
  return(H)
}
...
H_est <- function(phi1_interval_t, phi2_interval_t, psi1_interval_t,
                  x, weights, xi, zeta1, zeta2, zeta3, t){
  ...
  H <- exp(logh0 + sum(x*gamma_x))*t/2*sum(f*weights)
  return(H)
}

md_dp <- stan_model('source_code/dp_M21.stan')
...
for (Tstart in starting.time){
  ...
  stan_dat_dp <- list(n = length(tmp.ID.test), J = J, 
                      ...
                      L0 = L0, L1 = L1, P = P, 
                      P_x = ncol(x.test), #P_z = ncol(z.test), 
                      ...
                      gamma0 = gamma0, 
                      gamma1 = gamma1)
  inits1_dp <- inits2_dp <- inits3_dp <- inits4_dp <- list(xi = matrix(0, L0, length(tmp.ID.test)), 
                                 zeta1 = matrix(0, L1, length(tmp.ID.test)), 
                                 zeta2 = matrix(0, L1, length(tmp.ID.test)), 
                                 zeta3 = matrix(0, L1, length(tmp.ID.test)))
  inits_dp <- list(c1 = inits1_dp, c2 = inits2_dp, c3 = inits1_dp, c4 = inits2_dp)
  fitStan_dp <- sampling(md_dp, data = stan_dat_dp, iter = 4000, warmup = 3000, 
                         chains = 4, thin=1, init = inits_dp, pars = pars_dp, seed = 123,
                         control = list(adapt_delta = 0.8, max_treedepth=15))
  fname <- paste0("/path/dp_omics/stan_fit_2/1241_4000_3000_200_cph11_nc", args, '_dp.RData',sep="")
  save(list = 'fitStan_dp', file = fname)
...
result[[ii]] <- list(fitStan = fitStan,
                     fitStan_dp = fitStan_dp,
...
fname <- paste0("/path/dp_omics/result_M2_2/1241_4000_3000_200_cph11_nc", args, '.RData',sep="")
