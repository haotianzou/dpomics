...
data_gen_omics_negbin <- function(n_views=3, n_features=100, n_samples = 50, n_groups = 1,
                                  n_factors = 5, likelihood = "gaussian",
                                  lscales = 1, sample_cov = NULL, as.data.frame = FALSE) {
  
  # Sanity checks
  if (!all(likelihood %in% c("gaussian", "bernoulli", "poisson", 'negbin')))
  ...
  data <- lapply(seq_len(n_views), function(vw){
    ...
    if (lk == "gaussian"){
      ...
      term <- log(1+exp(mu[[vw]]))
      dd <- t(apply(term, 2, function(tt) MASS::rnegbin(length(tt),tt, theta = 4.5)))
    }
